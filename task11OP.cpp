...
string calculatePoolState(int volume,int pipe1,int pipe2,float hours)
{
	...
	if(volume<totalvolumefilled)
	{
	int overflow=totalvolumefilled-volume;
	string rstring = "For " + to_string(hours) +  " hours, the pool overflows with " + to_string(overflow) + " liters.";
	return rstring;
	}
}
main()
{	int volume,pipe1,pipe2;
	...
	cin >> hours;
	cout << calculatePoolState(volume,pipe1,pipe2,hours);
	}
